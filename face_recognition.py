        ...
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                if confidence > 77:
                    ...
                    cv2.putText(img, f"Accuracy:{confidence}", (x, y + 10), cv2.FONT_HERSHEY_COMPLEX, 0.8,(255, 255, 255), 3)
                    self.mark_attendance(i,r,n)

                    if id not in recognized_ids:
                        recognized_ids.add(id)
                        recognized_times[id] = time.time()
                        now = datetime.now()
                        date_str = now.strftime('%d-%m-%Y')
                    #
                    #     attendance = 'Present'
                    #     self.mark_attendance(i, n, r)
                        cursor.execute("INSERT INTO stdattendance(std_id, std_reg_no, std_name, std_time, std_date,std_attendance) "
                                           "VALUES (%s, %s, %s, NOW(), %s,'Awaited')", (i, r, n, date_str))
                        conn.commit()
                        conn.close()
                    #
                    #
                    else:
                         if (time.time() - recognized_times[id]) > 30:
                             self.mark_attendance(i,r,n)
                    #
                             cursor.execute("UPDATE stdattendance SET std_attendance='Present' WHERE std_id=%s", (id,))
                    #
                             conn.commit()
                else:
